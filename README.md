...
`5.`では，物理演算を適用したことにより重力が働き，下へ落下していったはずです！
...
`7.`ではコライダーによる衝突判定により，地面に落ちた際の挙動がリアルに再現されたはずです！
...
`3.`でダウンロードしたZIPファイルに含まれる`base` > `AerodynamicCalculator.cs`は，
...
![action-prop-use-reference](action-prop-use-reference.png)


右端の"◎（二重丸）"をクリックし，"Player/Move"をアタッチします．

![select-input-action-reference](select-input-action-reference.png)
![aircraft-inspector-final](aircraft-inspector-final.png)
...
![pratform-transform](pratform-transform.png)
